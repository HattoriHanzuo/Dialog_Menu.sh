...
do
	...
	then
		...
		then
			...
			then
				...
				then
					...
					if [ $a ];
			        	then
			        		passwd=`dialog --colors --passwordbox '\Z1Bitte das -sudo- Passwort eingeben' 0 0\
							3>&1 1>&2 2>&3`;
		...
		then
            ...
            then
            s=`dialog --menu 'NetCat Menü' 0 0 0\
                ...
                then
                    ...
                    if [ $s = '1' ]
                        then
                            ...
                            port=`dialog --inputbox 'Bitte Port zum lauschen eingeben' 0 70 \
                            3>&2 2>&1 1>&3`;

                            if [ $port ];
                                then
                                    dialog --scrollbar --prgbox test "nc -l $port" 30 60;
                        ...
                        then
                            port=`dialog --colors --inputbox 'Bitte \Zb\Z1Port\Z0\ZB zum lauschen eingeben' 0 70 \
                            3>&2 2>&1 1>&3`;
                            pfad=`dialog --dselect /home/$USER/ 10 70 \
                            3>&2 2>&1 1>&3`;

                            if [ $pfad ] && [ $port ];
                                then
                                    dialog --scrollbar --prgbox test "nc -l $port > '$pfad'" 60 90;
